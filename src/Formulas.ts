...
export const isErr = (v: Value): v is Err =>
...
type TokenType =
  ...
  | 'RIGHT_PAREN'
  | 'COMMA'
  | 'COLON' // keep ':' only if you want simple ranges inside SUM
  | 'OP'
  | 'EOF'
...
class Tokenizer {
  ...
  private skipWS() {
    while (/\s/.test(this.peek())) this.adv()
  }
  ...
  next(): Token {
    ...
    if (c === '(') {
      this.adv()
      return { type: 'LEFT_PAREN', value: '(', i }
    }
    if (c === ')') {
      this.adv()
      return { type: 'RIGHT_PAREN', value: ')', i }
    }
    if (c === ',') {
      this.adv()
      return { type: 'COMMA', value: ',', i }
    }
    if (c === ':') {
      this.adv()
      return { type: 'COLON', value: ':', i }
    }

    const pair = c + this.peek(1)
    if (twoOps.has(pair)) {
      this.adv(2)
      return { type: 'OP', value: pair, i }
    }
    if (oneOps.has(c)) {
      this.adv()
      return { type: 'OP', value: c, i }
    }
...
class Parser {
  ...
  private lbp(t: Token) {
    return t.type === 'OP' ? (BP[t.value] ?? 0) : t.type === 'COLON' ? BP_COLON : 0
  }
  ...
  private nud(t: Token): Expr {
    ...
    if (t.type === 'LEFT_PAREN') {
      const e = this.expr(0)
      const r = this.next()
      if (r.type !== 'RIGHT_PAREN') throw new Error(')')
      return e
    }
    if (t.type === 'OP' && (t.value === '+' || t.value === '-'))
      return { kind: 'Unary', op: t.value, arg: this.expr(75) }
    if (t.type === 'IDENT') {
      // function call?
...
export function defaultFns(): Record<string, (args: Value[]) => Value> {
  return {
    SUM(args) {
      return reduceArgs(args, (acc, x) => acc + x, 0)
    },
    MIN(args) {
      return reduceArgs(args, (acc, x) => Math.min(acc, x), +Infinity)
    },
    MAX(args) {
      return reduceArgs(args, (acc, x) => Math.max(acc, x), -Infinity)
    },
  }
}
